...
static void lis3dsh_init(void)
{
	...
	if( intRegValue == UC_WHO_AM_I )
	{
		
		LIS3DSH_write_reg(ADD_REG_CTRL_4, UC_ADD_REG_CTRL_4_CFG);
		LIS3DSH_write_reg(ADD_REG_CTRL_5, UC_ADD_REG_CTRL_5_CFG);
		
		intRegValue = LIS3DSH_read_reg(ADD_REG_CTRL_4);
		
		if( intRegValue != UC_ADD_REG_CTRL_4_CFG )
		{
			while(1);
		}
	}
...
static void Get_origin_value(){
		X_value = ((LIS3DSH_read_reg(ADD_REG_OUT_X_H) << 8) | LIS3DSH_read_reg(ADD_REG_OUT_X_L));
		X_value = data_transfer(X_value);
		X_value = X_value * SENS_RANGE;

		Y_value = ((LIS3DSH_read_reg(ADD_REG_OUT_Y_H) << 8) | LIS3DSH_read_reg(ADD_REG_OUT_Y_L));
		Y_value = data_transfer(Y_value);
		Y_value = Y_value * SENS_RANGE;

		Z_value = ((LIS3DSH_read_reg(ADD_REG_OUT_Z_H) << 8) | LIS3DSH_read_reg(ADD_REG_OUT_Z_L));
